...
def dessinerperdu():
    ...
    while continuer ==False :
        ...
        fenetre.blit(texteperdu4,(690,25)) #On affiche toujours le score final lors de la mort 
        pygame.display.flip()
        while True :
...
while continuer: # Cette boucle s'exécute jusqu'à ce que le joueur ferme la fenêtre
    ...
    elif random.randint(0, niveau(score)[1]) == 1:#S'occupe du spawn rate des fruits 
    ...
    for i in range(len(fruits)):
        ...
        fruits[i] = fruit     #je réactualise à la fin  comme je ne voulait pas multiplier les intendations inutiles 
    ...
    if click[0] == True :   #Ici j'ai décidé de n'effectuer ces conditions que si on clique pour ne pas mobilisé de la puissance de calcul pour rien
        for j in range(len(fruits)):
            hitbox = pygame.Rect(fruits[j][1][0],fruits[j][1][1], 383*0.25, 383*0.25)# Je définie la hitbox à la taille de mon sprite 
            if hitbox.collidepoint(slashs[-1][0],slashs[-1][1]) == True and fruits[j][7]=="fruit": #pour les les fruits de Type "FRUIT" lorsque le dernier point ajouté à notre liste de slash rencontre un fruit
                score +=1 
